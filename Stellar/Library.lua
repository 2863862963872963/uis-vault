...
RunService.Heartbeat:Connect(function()
    ...
    Countdown.Text = string.format("%02dh %02dm %02ds", hours, minutes, seconds)
        ...
        Library.Page = function(self, Data)
            ...
            function Page:SetCollapsed(Bool)
                Page.Collapsed = Bool

                -- Rotate the tab icon to hint collapse state
                Tween:Create(
                    Items["Icon"].Instance,
                    TweenInfo.new(Library.Tween.Time, Library.Tween.Style, Library.Tween.Direction),
                    { ImageTransparency = Bool and 0.7 or 0 },
                    true
                )

                Items["Page"].Instance.Visible = not Bool
            end

            Items["Inactive"]:Connect("MouseButton1Down", function()
                for Index, Value in Page.Window.Pages do 
                    if Value == Page and Page.Active then
                        Page:SetCollapsed(not Page.Collapsed)
                        return
                    end
        ...
        Library.Pages.Section = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["CollapseArrow"] = Instances:Create("ImageLabel", {
                    Parent = Items["Top"].Instance,
                    Name = "\0",
                    ImageColor3 = Library.Theme["Text"],
                    BorderColor3 = FromRGB(0, 0, 0),
            ...
            function Section:SetCollapsed(Bool)
                ...
                Tween:Create(
                    Items["CollapseArrow"].Instance,
                    TweenInfo.new(Library.Tween.Time, Library.Tween.Style, Library.Tween.Direction),
                    { Rotation = Bool and 90 or 0 },
                    true
                )
...
-- Window:AddTabSection(NameOrData, Icon)
-- Creates a new tab (page) AND a section inside it in one call.
-- Returns the section so elements can be added directly.
--
-- Usage:
--   local Section = Window:AddTabSection("Combat")
--   Section:AddToggle("Aimbot", ...)
--
-- With icon:
--   local Section = Window:AddTabSection({ Name = "Combat", Icon = "sword" })
--
-- With separate section name:
--   local Section = Window:AddTabSection({ Name = "Combat", SectionName = "Settings" })
--
function Library:AddTabSection(NameOrData, Icon)
    local Data = NormalizeNamedData(NameOrData, Icon)
    Data.Icon = self:ResolveIcon(Data.Icon or Data.icon)
    Data.icon = Data.Icon
    ...
    local Page
    if self.CreateTab then
        Page = self:CreateTab(Data)
    elseif self.Page then
        Page = self:Page(Data)
    else
        -- Fallback: this is a Library-level call — delegate through AddTab
        Page = self:AddTab(Data)
    end

    -- Build section data (optionally a different name for the section itself)
    local SectionData = {
        Name    = Data.SectionName or Data.sectionName or Data.Name or Data.name or "Section",
        Icon    = Data.SectionIcon or Data.sectionIcon or Data.Icon or "",
    }

    -- Create the section inside the tab
    local Section
    if Page.AddSection then
        Section = Page:AddSection(SectionData)
    elseif Page.Section then
        Section = Page:Section(SectionData)
    elseif Page.CreateSection then
        Section = Page:CreateSection(SectionData)
    end

    return Section
end

-- Also expose it on the CreateWindow result (Window table shares Library metatable,
-- so this lands on Window objects automatically via Library.__index).
Library.CreateTabSection = Library.AddTabSection
